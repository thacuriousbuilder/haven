...
export default function FoodDetailScreen() {
  ...
  const fetchFoodLog = async () => {
    try {
      ...
      setLogDate(data.log_date); // store for delete recalc
    ...
    { text: 'Cancel', style: 'cancel' },
    {
      ...
      onPress: async () => {
        try {
          ...
          // 1. Get the log_date BEFORE deleting (need it for sync)
          const { data: log } = await supabase
            .from('food_logs')
            .select('log_date')
            .eq('id', id)   // <-- whatever your ID variable is
            .single();

          // 2. Delete the food log
          ...
          // 3. Resync daily_summaries from remaining food_logs
          if (log?.log_date) {
...
const styles = StyleSheet.create({
  ...
  foodName: { fontSize: 24, fontWeight: '700', color: Colors.graphite, marginBottom: Spacing.xl },
  caloriesCard: {
  ...
  caloriesLabel: { fontSize: 14, fontWeight: '500', color: Colors.steelBlue, marginBottom: Spacing.xs },
  caloriesValue: { fontSize: 48, fontWeight: '700', color: Colors.graphite },
  macrosContainer: { flexDirection: 'row', gap: Spacing.md, marginBottom: Spacing.lg },
  macroCard: {
  ...
  macroLabel: { fontSize: 12, fontWeight: '500', color: Colors.steelBlue, marginBottom: 4 },
  macroValue: { fontSize: 20, fontWeight: '700', color: Colors.graphite },
  metaCard: {
  ...
  metaLabel: { fontSize: 14, fontWeight: '500', color: Colors.steelBlue },
  metaValue: { fontSize: 14, fontWeight: '600', color: Colors.graphite },
  metaDivider: { height: 1, backgroundColor: Colors.border, marginVertical: Spacing.md },
  editButton: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
    gap: Spacing.sm, backgroundColor: Colors.vividTeal,
    paddingVertical: Spacing.lg, borderRadius: BorderRadius.lg, ...Shadows.small,
  },
  editButtonText: { fontSize: 16, fontWeight: '600', color: Colors.white },
});